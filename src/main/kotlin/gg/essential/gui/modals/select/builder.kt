...
import gg.essential.gui.modals.select.component.groupIcon
...
class SelectModalBuilder<T>(
    ...
    private val relationshipStates = RelationshipStateManagerImpl(connectionManager.relationshipManager)
    private val messageStates = MessengerStateManagerImpl(connectionManager.chatManager)
    private val statusStates = StatusStateManagerImpl(connectionManager.profileManager, connectionManager.spsManager)
    ...
    internal val defaultUserOrGroupRow: SectionLayoutBlock<Channel>
        get() = { selected, channel ->
            val otherUser = channel.getOtherUser()
            row(Modifier.fillParent(padding = 3f)) {
                if (otherUser == null) {
    ...
    fun emptyText(text: State<String>) = apply {
        whenEmpty = {
            column {
                spacer(height = 3f)
                EssentialMarkdown(text, emptyTextMarkdownConfig)(Modifier.width(130f)).onLinkClicked(essentialUriListener)
            }
        }
    }
    ...
    fun LayoutScope.groupEntry(selected: MutableState<Boolean>, id: Long) {
        row(Modifier.fillRemainingWidth(), Arrangement.spacedBy(5f)) {
            groupIcon(id, modifier = Modifier.width(8f).heightAspect(1f))
    ...
    fun users(
        ...
        section(
            ...
            searchFilter = { identifier, searchText ->
                UUIDUtil.nameState(identifier).contains(searchText, ignoreCase = true)
            },
            block = block
        )
    }
    ...
    fun filterFriendsByActivity(predicate: (PlayerActivity) -> Boolean): ListState<UUID> {
        val mappedFriends = relationshipStates.getObservableFriendList()
            .toStateV2List()
            .mapEach { uuid ->
                uuid to statusStates.getActivityState(uuid).map { predicate(it) }.toV2()
            }
