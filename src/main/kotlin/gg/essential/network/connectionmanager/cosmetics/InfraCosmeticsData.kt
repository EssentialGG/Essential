...
class InfraCosmeticsData private constructor(
    ...
    constructor(connectionManager: ConnectionManager, assetLoader: AssetLoader) : this(connectionManager, assetLoader, MutableCosmeticsData())
    ...
    fun addCosmetic(infraCosmetic: InfraCosmetic) {
        ...
        val assets = infraCosmetic.assets?.toMod(infraCosmetic.assetsMap?.mapValues { it.value.toMod() }) ?: return
        ...
        val settingsFuture = if (settingsAsset != null) {
            ...
            assetLoader.getAssetBytes(settingsAsset, AssetLoader.Priority.Blocking)
                ...
                .whenCompleteAsync({ _, _ -> cosmeticsLoading.remove(infraCosmetic.id) }, Minecraft.getMinecraft().executor)
                .logExceptions()
        } else {
    ...
    fun requestCategoriesIfMissing(categoryIds: Collection<CosmeticCategoryId>) {
        ...
        connectionManager.send(
            ClientCosmeticCategoriesRequestPacket(unknownIds.toSet(), null, null),
            { _ ->
                for (id in unknownIds) {
                    activeCategoryRequests.remove(id)
                }
            },
            TimeUnit.SECONDS, CosmeticsManager.LOAD_TIMEOUT_SECONDS,
        )
    }
    ...
    fun requestCosmeticsIfMissing(cosmeticIds: Collection<CosmeticId>) {
        ...
        connectionManager.send(
            ClientCosmeticRequestPacket(unknownIds.toSet(), null),
            { _ ->
                for (id in unknownIds) {
    ...
    fun requestBundlesIfMissing(bundleIds: Collection<CosmeticBundleId>) {
        ...
        connectionManager.send(
            ClientWardrobeStoreBundleRequestPacket(unknownIds.toSet()),
            { _ ->
                for (id in unknownIds) {
