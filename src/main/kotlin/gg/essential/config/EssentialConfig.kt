...
object EssentialConfig : Vigilant2(), GuiEssentialPlatform.Config {
    ...
    fun gui(): VigilanceV2SettingsGui = (gui)() // to keep existing use from Java the same
    ...
    fun hookUp() {
        ...
        ownCosmeticsHiddenStateWithSource.onSetValue(referenceHolder) { (hidden, setByUser) ->
            if (Essential.getInstance().connectionManager.isAuthenticated) {
                ...
                this.ownCosmeticsHidden = !hidden
            }
        }
    ...
    fun getCosmeticArmorSetting(entity: Entity): Int {
        return if (entity is EntityPlayerSP) {
            cosmeticArmorSettingSelf
        } else {
            cosmeticArmorSettingOther
        }
    }
    ...
    private fun toggleEssential(enabling: Boolean) {
        ...
        this.essentialEnabled = enabling
