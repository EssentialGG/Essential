/*
 * Copyright (c) 2024 ModCore Inc. All rights reserved.
 *
 * This code is part of ModCore Inc.'s Essential Mod repository and is protected
 * under copyright registration # TX0009138511. For the full license, see:
 * https://github.com/EssentialGG/Essential/blob/main/LICENSE
 *
 * You may not use, copy, reproduce, modify, sell, license, distribute,
 * commercialize, or otherwise exploit, or create derivative works based
 * upon, this file or any other in this repository, all of which is reserved by Essential.
 */
package gg.essential.network.connectionmanager.ice.util;

import java.util.Comparator;

/**
 * Comparator which only returns 0 if {@link Object#equals(Object)} is true.
 * Objects which appear to be equal according to the given comparator but not their equals method are compared by their
 * identity hash code.
 */
public class StrictComparator<T> implements Comparator<T> {

    private final Comparator<T> comparator;

    public StrictComparator(Comparator<T> comparator) {
        this.comparator = comparator;
    }

    @Override
    public int compare(T o1, T o2) {
        int result = comparator.compare(o1, o2);
        if (result == 0 && !o1.equals(o2)) {
            result = Integer.compare(System.identityHashCode(o1), System.identityHashCode(o2));
        }
        return result;
    }
}
