...
class ParagraphDrawable(
    ...
    override fun layoutImpl(x: Float, y: Float, width: Float): Layout {
        ...
        fun layout(drawable: Drawable, width: Float) {
            drawable.layout(currX, currY, width).also {
        ...
        fun checkThenTrimTextDrawable(textDrawable: TextDrawable) {
            if (trimNextText) {
                textDrawable.ensureTrimmed()
                trimNextText = false
            }
        }
        ...
        for ((index, text) in originalDrawables.withIndex()) {
            ...
            while (true) {
                ...
                checkThenTrimTextDrawable(target)
                ...
                if (targetWidth > width) {
                    ...
                    if (splitResult2 == null) {
                        ...
                        val currentLineResult = splitResult3.first
                        checkThenTrimTextDrawable(currentLineResult)
                        layout(currentLineResult, currentLineResult.width())
                        gotoNextLine()
                    ...
                    val currentLineResult = splitResult2.first
                    checkThenTrimTextDrawable(currentLineResult)
                    layout(currentLineResult, currentLineResult.width())
                    gotoNextLine()
    ...
    override fun cursorAt(mouseX: Float, mouseY: Float, dragged: Boolean, mouseButton: Int): Cursor<*> {
        ...
        currentDrawable.style.linkLocation?.takeIf { !dragged && mouseButton == 0 }?.let { linkLocation ->
            if (md.fireLinkClickEvent(EssentialMarkdown.LinkClickEvent(linkLocation))) {
                try {
