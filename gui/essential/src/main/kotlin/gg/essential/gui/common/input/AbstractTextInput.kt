...
abstract class AbstractTextInput(
    ...
    protected var targetVerticalScrollingOffset = 0f
        set(value) {
            // Since we added padding, disallow the target scroll offest to be bigger than the difference in height
            // This prevents us from scrolling higher and revealing the padding below the lowset line, which should be ignored
    ...
    private fun showOptionMenu(posX: Float, posY: Float) {
        ...
        if (hasSelection()) {
            options.add(ContextOptionMenu.Option("Copy", image = EssentialPalette.COPY_10X7) {
                copySelection()
            })
            options.add(ContextOptionMenu.Option("Cut", image = EssentialPalette.CUT_10X7) {
                copySelection()
                deleteSelection()
            })
        }
        options.add(ContextOptionMenu.Option("Paste", image = EssentialPalette.PASTE_10X8) {
            if (hasSelection()) {
                deleteSelection()
            }
